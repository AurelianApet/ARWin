            ...
            if (Physics.Raycast(ray, out hit))
            {
                if (hit.transform.gameObject.name == this.name)
                {
                    if (!isChromakey)
                    {
                        ...
                        GameObject videoControlObject = this.transform.Find("VideoControl").gameObject;
...
#endif
        Debug.Log(Application.persistentDataPath + "/Data" + strURL.Substring(strURL.LastIndexOf("/")));
